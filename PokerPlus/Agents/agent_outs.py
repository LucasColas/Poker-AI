...
class agent_outs:
    ...
    def action_river(self, good_hand):
        ...
        if self.__game.players[self.__game.current_player].state == PlayerState.IN:
            ...
            (
                self.__game.players[self.__game.current_player].state
                == PlayerState.TO_CALL
            )
            and good_hand
            and (self.__min_raise < self.__max_raise)
        ):
            # print("call, p =" ,p, "p_win=",p_win)
            action_type = ActionType.RAISE
            self.__total = self.__max_raise
        elif (
            self.__game.players[self.__game.current_player].state == PlayerState.TO_CALL
        ) and (self.__min_raise < self.__max_raise):
            rank = evaluate(
                self.__game.hands[self.__game.current_player], self.__game.board
            )
            p_win = get_five_card_rank_percentage(rank)
            p = random.random()

            if (
                (self.__max_raise > self.__min_raise)
                and (
                    self.__game.players[self.__game.current_player].state
                    == PlayerState.TO_CALL
                )
                ...
                if (self.__game.big_blind * 3) > self.__min_raise:
                    self.__total = random.randint(
                        self.__min_raise, self.__game.big_blind * 3
                    )
    ...
    def raise_config(self):
        ...
        self.__min_raise = self.__game.value_to_total(
            self.__game.min_raise(), self.__game.current_player
        )
    ...
    def choix(self, g: TexasHoldEm):
        ...
        if len_cards_game_board == 0:
            ...
            other_cards = [
                card
                for card in cards
                if card not in self.__agent_cards and card not in self.__game.board
            ]
            ...
            for id, combinaison in enumerate(combinaisons):
                ...
                self.__info_combi[id] = sorted(
                    self.__info_combi[id].items(), key=lambda item: item[0]
                )
                # print(self.__info_combi[id])

            chance = (
                self.__best_possible_hand[list(self.__best_possible_hand.keys())[0]] * 2
            )
            # print("chance : ", chance)
            ...
            if self.__game.players[self.__game.current_player].state == PlayerState.IN:
                ...
                if (
                    ...
                    self.__total = random.randint(
                        self.__min_raise,
                        min(self.__game.big_blind * 3, self.__max_raise),
                    )
                    self.__nb_check_consecutifs = 0
                else:
                    action_type = ActionType.CHECK
                ...
                if 0.5 * self.__max_raise > self.__min_raise:
                    self.__total = random.randint(
                        self.__min_raise, int(0.5 * self.__max_raise)
                    )
                else:
                    self.__total = random.randint(self.__min_raise, self.__max_raise)
            else:
