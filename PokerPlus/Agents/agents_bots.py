...
def agent_allIn(game: TexasHoldEm, seuil: int = 0.1):  # Agressif et large
    ...
    if len(game.board) == 0:
        ...
        if game.players[game.current_player].state == PlayerState.IN and (
            max_raise > min_raise
        ):
...
def agent_saboteur(game: TexasHoldEm):
    ...
    # Strategie : joue si il a que moins de 5% de chance de gagner
        ...
        if (
            ...
            action_type = ActionType.RAISE
            total = min_raise
        elif (game.players[game.current_player].state == PlayerState.TO_CALL) and (
            p_win < 0.05
        ):
            action_type = ActionType.CALL
