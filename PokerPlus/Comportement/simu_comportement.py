...
def simu_comportement(
    nb_tournoi: int = 50,
    max_players: int = 8,
    big_blind: int = 150,
    small_blind: int = 150 // 2,
    buyin: int = 1000,
    bots=[
        random_agent,
        agent_outs().choix,
...
def write_data_comportement(
    data_dict: dict,
    max_players=8,
    filename: str = "data_comportement.csv",
    path: str = "",
    bots_noms=[
        "random_agent",
        "agent_out",
    ...
    with open(path + filename, "w") as csvfile:
        ...
        for i in range(max_players):
            ...
            vpip = getVpip(
                data_dict["nbrCall"][(i, bots_noms[i])],
                data_dict["nbrRaise"][(i, bots_noms[i])],
                data_dict["nbrFold"][(i, bots_noms[i])],
                data_dict["nbrAction"][(i, bots_noms[i])],
            )
            ratio_large = getRatioLarge(
                data_dict["nbrFold"][(i, bots_noms[i])],
                data_dict["nbrAction"][(i, bots_noms[i])],
            )
            writer.writerow(
                {
                    fieldnames[0]: vpip,
                    fieldnames[1]: ratio_large,
                    fieldnames[2]: bots_noms[i],
                }
            )


def write_data_comportement2(
    data_dict: dict,
    nb_tournoi: int,
    filename: str = "data_comportement.csv",
    path: str = "",
    ...
    for i in range(nb_tournoi):
        ...
        nb_a = {
            j: data_dict["nbrAction"][f"tournoi {i}"][j] for j in range(max_players)
        }
    ...
    with open(path + filename, "w") as csvfile:
        ...
        for i in range(max_players):
            for k in range(nb_tournoi):
                writer.writerow(
                    {
                        fieldnames[0]: vpip[i][k],
                        fieldnames[1]: ratio_large[i][k],
                        fieldnames[2]: bots_noms[i],
                    }
                )
