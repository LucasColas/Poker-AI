...
def get_data(m: int = 2, max_players=6) -> dict:
    ...
    for i in range(m):
        data_dict[i] = get_stat_tournoi(
            nmax=1, poolrandom=True, max_players=max_players, verbose=True
        )
...
def write_data(
    m: int, max_players: int = 9, filename: str = "data_trial.csv", path: str = ""
):
    ...
    with open(path + filename, "w") as csvfile:
        ...
        for i in range(m):
            ...
            vpip_ = vpip(
                data_dict[i]["nbrCall"][f"tournoi {nb_tournoi}"],
                data_dict[i]["nbrRaise"][f"tournoi {nb_tournoi}"],
                data_dict[i]["nbrAction"][f"tournoi {nb_tournoi}"],
                data_dict[i]["nbrFold"][f"tournoi {nb_tournoi}"],
                max_players,
            )
            ratio_large_ = ratio_large(
                data_dict[i]["nbrFold"][f"tournoi {nb_tournoi}"],
                data_dict[i]["nbrPartie"][f"tournoi {nb_tournoi}"],
                max_players,
            )
